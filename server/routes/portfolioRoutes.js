...
function syntheticPriceFromAverage(symbol, avgPrice) {
  ...
  return Number((avgPrice * (1 + drift)).toFixed(2));
}
...
async function trade(req, res, side) {
  ...
  if (!live) return res.status(404).json({ error: "Unknown symbol" });
  ...
  if (side === "buy") {
    ...
    if (idx === -1) {
      ...
      h.avgPrice = (h.qty * h.avgPrice + quantity * price) / newQty;
      h.qty = newQty;
    }
  } else {
  ...
  portfolio.transactions.unshift({ symbol: sym, side, qty: quantity, price, at: new Date() });
